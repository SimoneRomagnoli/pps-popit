...
object RegeneratingBalloons {
  ...
  trait Regenerating extends Balloon {
    ...
    private def regenerate(dt: Double): Regenerating = timer - dt match {
      case t if t <= 0 && maxLife > this.life => timer = regenerationTime; new RegeneratingComplex(this)
      case t => println(t); timer = t; this
    }
    override def update(dt: Double): Regenerating = regenerating(super.update(dt).asInstanceOf[Balloon]).regenerate(dt)
  }

  class RegeneratingSimple(override val position: Vector2D = (0.0, 0.0),
                           override val speed: Vector2D = (0.0, 0.0)) extends Simple(position, speed) with Regenerating
  class RegeneratingComplex(override val balloon: Balloon) extends Complex(balloon) with Regenerating
