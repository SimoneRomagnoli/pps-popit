...
object RoundsFactory {
  ...
  private def easyRounds(round: Int): IO[Unit] = for {
    _ <- add((Streak(if (round <= 7) 5 * round else 5 * (easyRoundLimit - round)) :- Red) @@ (1000 / (round % 5 + 1)).milliseconds)
    _ <- if(round >= 5) add((Streak(round) :- Blue) @@ (1000 / (round % 5 + 1)).milliseconds) else IO.unit
    _ <- if(round >= 10) add((Streak(round / 2) :- Green) @@ (1000 / (round % 5 + 1)).milliseconds) else IO.unit
  } yield ()
