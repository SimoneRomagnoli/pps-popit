...
import controller.Messages.{BalloonMoved, CollisionDetected, EntityUpdated, Input, SearchBalloon, Start, Tick, TickUpdate, Update, UpdateEntity, UpdatePosition}
import model.TowersTest.{Balloon, balloon, balloonPosition, collided, dummyBalloonActor, dummyModel, tower, towerPosition, waitSomeTime}
...
object TowersTest {
  ...
  val dummyBalloonActor: Balloon => Behavior[Update] = balloon => Behaviors.receiveMessage {
  ...
  val dummyModel: ActorRef[Update] => Behavior[Update] = tower => Behaviors.setup { ctx =>
...
class TowersTest extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  val balloonActor: ActorRef[Update] = testKit.spawn(dummyBalloonActor(Balloon(1.0, balloonPosition)))
