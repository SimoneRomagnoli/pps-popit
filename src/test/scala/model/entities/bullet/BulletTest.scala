...
import model.entities.bullet.Bullets.{ CannonBall, Dart, IceBall }
...
class BulletTest extends AnyFlatSpec with Matchers {
  ...
  val iceBall: IceBall = IceBall(damage, position, speed, radius, freezingTime)

  val dart: Dart = Dart(damage, position, speed)
  val cannonBall: CannonBall = CannonBall(damage, position, speed, radius)
